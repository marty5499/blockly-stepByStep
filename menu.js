...
class AIMenu {
  constructor() {
    ...
    this.data = {}
    for (var i = 0; i < this.list.length; i++) {
      this.data[this.list[i].innerHTML] = new Item(this.list[i].parentElement.parentElement);
    }
  }
  open(str) {
    return this.data[str];
...
await block.moveTo(300, 50,attach = false);

var block = menu.open('矩陣 LED').click().select(1);
await block.mouseOver();
await block.moveTo(360, 40,attach = true);

var block = menu.open('矩陣 LED').click().select(2);
await block.mouseOver();
await block.moveTo(360, 0,attach = true);